...
public class BinaryTree<Anytype> {
    ...
    public int Sum() {
        if (Root !=null)
            return Sum(Root);  // Method 1
          // return Root.Sum();  // Method 2
        else return 0;
    }
    
    // Method 1 - recursive helper
    private int Sum(BinaryNode currentRoot) {
        int s = (int)currentRoot.getElement();
        
        if (currentRoot.getLeft()!=null)
            s+= Sum(currentRoot.getLeft());
        
        if (currentRoot.getRight()!=null)
            s+= Sum(currentRoot.getRight());
